    ...
    <div className="patient-record-container">
      ...
      <div className="pr-header">
        <div className="pr-header-left">
          <button className="pr-back-icon" onClick={() => navigate('/admin/patients')} title="Back to Patients">
            <ArrowLeft size={20} />
          </button>
          <div>
  ...
  return (
    <div>
      ...
      <div className="pr-info-grid">
        ...
        <div className="info-group"><label>Date of Birth</label><p>{patient.date_of_birth ? new Date(patient.date_of_birth).toLocaleDateString() : 'â€”'}</p></div>
        <div className="info-group"><label>Gender</label><p style={{ textTransform: 'capitalize' }}>{patient.gender}</p></div>
        <div className="info-group"><label>Civil Status</label><p style={{ textTransform: 'capitalize' }}>{patient.civil_status || 'â€”'}</p></div>
        <div className="info-group"><label>Blood Type</label><p>{patient.blood_type || 'â€”'}</p></div>
        ...
        <div className="info-group" style={{ gridColumn: '1 / -1' }}><label>Address</label><p>{patient.address || 'â€”'}</p></div>

        <div className="info-group"><label>Emergency Contact Name</label><p>{patient.emergency_contact_name || 'â€”'}</p></div>
        <div className="info-group"><label>Emergency Contact Number</label><p>{patient.emergency_contact_number || 'â€”'}</p></div>
        ...
        {consultations.map(c => (
          <div key={c.id} className="pr-history-card">
            ...
            <div className="pr-card-body">
              ...
              <p><strong>Department:</strong> {c.department || 'â€”'}</p>
              <p><strong>Chief Complaint:</strong> {c.chief_complaint || 'â€”'}</p>
              <p><strong>Diagnosis:</strong> {c.assessment || 'â€”'}</p>
            </div>
          </div>
        ))}
      </div>
        ...
        {admissions.map(a => (
          <div key={a.id} className="pr-history-card">
            ...
            <div className="pr-card-body">
              ...
              <p><strong>Reason for Admission:</strong> {a.reason_for_admission || 'â€”'}</p>
            </div>
          </div>
        ))}
      </div>
        ...
        {prescriptions.map(p => (
          <div key={p.id} className="pr-history-card">
            ...
            <div className="pr-card-body">
              ...
              <p><strong>Notes:</strong> {p.notes || 'â€”'}</p>
        ...
        <tbody>
          {schedules.map(s => (
            <tr key={s.id}>
              ...
              <td>{s.actual_administration_time ? new Date(s.actual_administration_time).toLocaleString() : 'â€”'}</td>
              <td>{s.administeredBy ? `Nurse ${s.administeredBy.last_name}` : 'â€”'}</td>
              <td>
      ...
      <div className="pr-list-container">
        {discharges.map(d => (
          <div key={d.id} className="pr-history-card">
            ...
            <div className="pr-card-body">
              <p><strong>Final Diagnosis:</strong> {d.final_diagnosis || 'â€”'}</p>
              <p><strong>Condition upon Discharge:</strong> {d.condition_at_discharge || 'â€”'}</p>
              <p><strong>Summary:</strong> {d.discharge_summary || 'â€”'}</p>
              <p><strong>Follow-up Date:</strong> {d.follow_up_date ? new Date(d.follow_up_date).toLocaleDateString() : 'â€”'}</p>
              <p><strong>Follow-up Instructions:</strong> {d.follow_up_instructions || 'â€”'}</p>
            </div>
          </div>
        ))}
      </div>
